...
class AlertClass:
    def alert(uudet_pelit_id, session):
        headers = veikkausbot.headers
        bot = veikkausbot.bot
        ...
        for event_id in uudet_pelit_id:
            ...
            for kohde in response_dict:
                ...
                if (kohde['rows'][0]['type'] == '12'):
                    peli_info = parse.ParserClass.one_two_builder(kohde)
                ...
                if (kohde['rows'][0]['type'] == '1X2' or kohde['rows'][0]['type'] == 'AWAY_HANDICAP' 
                    ...
                    peli_info = parse.ParserClass.one_x_two_builder(kohde)
                    kohteet.append(peli_info)
                # Luo tietue yli/alle kohteille
                if (kohde['rows'][0]['type'] == 'OVER_UNDER'):
                    peli_info = parse.ParserClass.both_teams_to_score(kohde)
                    kohteet.append(peli_info)
                # Luo tietue "Molemmat tekevät maalin kohteille"
                if (kohde['rows'][0]['type'] == 'BOTH_TEAMS_TO_SCORE'):
                    peli_info = parse.ParserClass.over_under_builder(kohde)
                    kohteet.append(peli_info)
            if (len(kohteet) > 0):
